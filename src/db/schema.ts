...
export const subscriptionPlans = sqliteTable('subscription_plans', {
    ...
    termlyPrice: integer('termly_price').notNull(),
    annualPrice: integer('annual_price').notNull(),
    studentLimit: integer('student_limit').default(0), // 0 or -1 for unlimited
    isCloudEnabled: integer('is_cloud_enabled', { mode: 'boolean' }).default(false),
    isMultiUserEnabled: integer('is_multi_user_enabled', { mode: 'boolean' }).default(false),
    createdAt: text('created_at').default(sql`CURRENT_TIMESTAMP`),
});
...
export const subscriptions = sqliteTable('subscriptions', {
    ...
    billingCycle: text('billing_cycle').notNull(), // Termly, Annual
...
export const users = sqliteTable('users', {
    id: integer('id').primaryKey({ autoIncrement: true }),
    schoolId: integer('school_id').references(() => schools.id), // Null for super admins
    username: text('username').notNull().unique(),
    passwordHash: text('password_hash').notNull(),
    fullName: text('full_name').notNull(),
    role: text('role').notNull().default('teacher'), // super_admin, admin, teacher, bursar
    email: text('email'),
...
export const roles = sqliteTable('roles', {
    id: integer('id').primaryKey({ autoIncrement: true }),
    schoolId: integer('school_id').references(() => schools.id), // Null for system roles
    name: text('name').notNull(),
...
export const settings = sqliteTable('settings', {
    id: integer('id').primaryKey({ autoIncrement: true }),
    schoolId: integer('school_id').references(() => schools.id).notNull(),
    key: text('key').notNull(), // Removed unique() because keys can be same across schools
    value: text('value'),
